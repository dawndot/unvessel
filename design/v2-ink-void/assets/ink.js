...
(function () {
  ...
  var FRAG_SRC = [
    ...
    'void main() {',
    // 以短边为基准归一化坐标，画面中心为原点（保证任何宽高比下墨形不拉伸）
  ...
  var mouse = { x: 0.62, y: 0.42 };          // 默认偏置点：避免开局涡流正对中心
  var target = { x: 0.62, y: 0.42 };
  ...
  // 窗口尺寸变化：resize 在下一帧的 resize() 里自然完成，无需额外监听
  // （非动效模式下窗口缩放不需要重画——静帧拉伸可接受）

  /* ---------- 上下文丢失/恢复（GPU 重置、驱动切换等） ---------- */
